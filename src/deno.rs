...
use deno_core::{JsRuntime, RuntimeOptions, FastString, Extension};
use deno_core::v8;
    ...
    Extension {
        ...
        js_files: std::borrow::Cow::Borrowed(&[
            ...
            deno_core::ExtensionFileSource {
                specifier: "log.js",
                code: deno_core::ExtensionFileSourceCode::IncludedInBinary(r#"
...
pub fn init_plugin(plugin_path: &Path) -> Result<DenoPlugin> {
    ...
    runtime.execute_script("<anon>", FastString::from(js_code.to_string()))
...
pub fn transform_message(plugin: &mut DenoPlugin, message: &Value) -> Result<Value> {
    ...
    // Execute the JavaScript code
    let result = plugin.runtime.execute_script("<transform>", FastString::from(js_code.to_string()))
    ...
    let transform_result: TransformResult = serde_json::from_str(&result_str)
        .context("Failed to parse JavaScript result as JSON")?;
